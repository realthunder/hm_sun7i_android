...
int dump_qmux(QMI_SERVICE_TYPE serviceType, PQCQMUX_HDR QMUXHdr) {
    ...
    switch (serviceType) {
        ...
        case QMUX_TYPE_CTL:                
        default:
            dbg("Type:               %04x\t%s\n", le16_to_cpu(QMUXMsgHdr->Type), "PDS/QOS/CTL/unkonw!");
        break;    
    }
...
void dump_qmi(void *dataBuffer, int dataLen) 
{
    ...
    //printf("QCQMI_HDR-----------------------------------------");
    //printf("IFType:             %02x\t\t%s", QMIHdr->IFType, QMI_NAME(qmi_IFType, QMIHdr->IFType));
    //printf("Length:             %04x", QMIHdr->Length);
    //printf("CtlFlags:           %02x\t\t%s", QMIHdr->CtlFlags, QMI_NAME(qmi_CtlFlags, QMIHdr->CtlFlags));
    //printf("QMIType:            %02x\t\t%s", QMIHdr->QMIType, QMI_NAME(qmi_QMIType, QMIHdr->QMIType));
    //printf("ClientId:           %02x", QMIHdr->ClientId);

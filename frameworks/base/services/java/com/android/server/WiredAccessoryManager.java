...
final class WiredAccessoryManager implements WiredAccessoryCallbacks {
    ...
    private void setDeviceStateLocked(int headset,
        ...
        if ((headsetState & headset) != (prevHeadsetState & headset)) {
            ...
            if (LOG)
                Slog.v(TAG, "device ="+device+" ; " + headsetName+((state == 1) ? " connected" : " disconnected"));

            mAudioManager.setWiredDeviceConnectionState(device, state, headsetName);
			String keyValuePairs = "inputdevice=" + device + ";state=" + state;
    ...
    class WiredAccessoryObserver extends UEventObserver {
        ...
        private List<UEventInfo> makeObservedUEventList() {
            ...
            /* delete by chenjd, 2014.02.14, start---{{{
             * let the multi audio manager to due with plug in/out of headphone and usb audio, see AudioDeviceManagerObserver.java
             */

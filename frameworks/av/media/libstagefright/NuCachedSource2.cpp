...
void PageCache::copy(size_t from, void *data, size_t size) {
  ...
  if(from + size >mTotalSize )
  	return ;
//yf    CHECK_LE(from + size, mTotalSize);
...

    {
        Mutex::Autolock autoLock(mLock);
		  if(mFinalStatus != OK && mNumRetriesLeft <= 0 )
  			return ;
      //yf   CHECK(mFinalStatus == OK || mNumRetriesLeft > 0);
...
void NuCachedSource2::onFetch() {
    ...
    if (mFetching || keepAlive) {
        ...
        if (mFetching && mCache->totalSize() >= mHighwaterThresholdBytes) {
            ...
            mFetching = false;
			  status_t lStatus;
		if( approxDataRemaining_l(&lStatus) < mHighwaterThresholdBytes  )
			mFetching = true;//yf
                   ALOGV("continue dowload");
            if (mDisconnectAtHighwatermark
...
ssize_t NuCachedSource2::readAt(off64_t offset, void *data, size_t size) {
    ...
    ALOGV("readAt offset %lld, size %lu", offset, size);
...
//yf    CHECK(mAsyncResult == NULL);
	...
	if (!mAsyncResult->findInt32("result", &result)) {
		result = -1;
	}
...
ssize_t NuCachedSource2::readInternal(off64_t offset, void *data, size_t size) {
	if(size > (size_t)mHighwaterThresholdBytes )
		return -1;
  //yf  CHECK_LE(size, (size_t)mHighwaterThresholdBytes);
...
status_t NuCachedSource2::seekInternal_l(off64_t offset) {
	...
	if(mCache->releaseFromStart(totalSize) > totalSize)
		return OK;
   //yfCHECK_EQ(mCache->releaseFromStart(totalSize), totalSize);

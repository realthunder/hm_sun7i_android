...
protected int mPreviousConfigMcc;
    ...
    /**
     * it can add a item to filter list or delete a item from filter list
     * @param cn the ComponentName you want to add/del
     * @param isAdd add/del flag
     */
    static public void setFavoritesApp(Context context ,final ComponentName cn,final boolean isAdd){
        long time = System.currentTimeMillis();
        String packageName = cn.getPackageName();
        String className = cn.getClassName();
        ContentValues values = new ContentValues();
        values.put(LauncherSettings.FavoritesApp.PACKAGE_NAME, packageName);
        values.put(LauncherSettings.FavoritesApp.CLASS_NAME, className);
        
        final ContentResolver cr = context.getContentResolver();
        Uri result = cr.insert(LauncherSettings.FavoritesApp.CONTENT_URI, values);
        if(result != null){
            Toast.makeText(context, context.getString(R.string.add_favorites_app_success), 
                    Toast.LENGTH_LONG).show();
        }
        Log.d(TAG,"save favoriteApp time" + (System.currentTimeMillis()-time) + "ms");
    }
    
    static public void delFavoritesAppFromDataBase(final Context context, final ComponentName cn){
        
        final ContentResolver cr = context.getContentResolver();
        final Cursor cursor = cr.query(LauncherSettings.FavoritesApp.CONTENT_URI, 
                new String[]{LauncherSettings.FavoritesApp._ID}, 
                LauncherSettings.FavoritesApp.PACKAGE_NAME + "=\""+cn.getPackageName() + 
                "\"" + " and " + 
                LauncherSettings.FavoritesApp.CLASS_NAME + "=\""+cn.getClassName()+"\"", 
                null, null);
            ...
            sWorker.post(new Runnable() {
                public void run() {
                    ...
                    do{
                        int id;
                        id = cursor.getInt(0);
                        final Uri delUri = LauncherSettings.FavoritesApp.getContentUri(id, true);
                        result |= (cr.delete(delUri, null, null) > 0);
                    }while(cursor.moveToNext());
                    if(result){
                        Toast.makeText(context, context.getString(R.string.del_favorites_app_success), 
                                Toast.LENGTH_LONG).show();
                    }
                    cursor.close();
                }
 ...
 private void forceReload() {
     ...
     */
    static public void addOrMoveItemInDatabase(Context context, ItemInfo item, long container,
     ...
     */
    static public void moveItemInDatabase(Context context, ItemInfo item, long container, int screen,
            int cellX, int cellY) {
     ...
     */
    static public boolean shortcutExists(Context context, String title, Intent intent) {
        final ContentResolver cr = context.getContentResolver();
     ...
     */
    static public void addItemToDatabase(Context context, ItemInfo item, long container,
     ...
     */
    static public void updateItemInDatabase(Context context, ItemInfo item) {
        final ContentValues values = new ContentValues();
        final ContentResolver cr = context.getContentResolver();
     ...
     * @param item
     */
    static public void deleteItemFromDatabase(Context context, ItemInfo item) {
        final ContentResolver cr = context.getContentResolver();
     ...
     */
    static public void deleteUserFolderContentsFromDatabase(Context context, UserFolderInfo info) {
        final ContentResolver cr = context.getContentResolver();
    ...
    private class LoaderTask implements Runnable {
        ...
        private boolean loadFavoritesApp(){
            long time = System.currentTimeMillis();
            Context context = mContext;
            final ContentResolver contentResolver = context.getContentResolver();
            final Cursor c = contentResolver.query(
                    LauncherSettings.FavoritesApp.CONTENT_URI, null, null, null, null);
            c.moveToFirst();
            final int indexPackageName = c.getColumnIndexOrThrow(
                    LauncherSettings.FavoritesApp.PACKAGE_NAME);
            final int indexClassName = c.getColumnIndexOrThrow(
                    LauncherSettings.FavoritesApp.CLASS_NAME);
            ComponentName cn;
            String packageName;
            String className;
            synchronized(mFilterList){
                ...
                try{
                    ...
                    do{
                        ...
                        packageName = c.getString(indexPackageName);
                        className = c.getString(indexClassName);
                        cn = new ComponentName(packageName,className);
                        if(true) Log.d(TAG,"dump favorites app info " + cn.toString());
                        if(!mFilterList.contains(cn)){
                            mFilterList.add(cn);
                        }
                    }while(c.moveToNext());
                } catch(Exception e){
                    Log.w(TAG,"loading favorites application interrupted" +
                    		e);
                }finally{
                    c.close();
                }
            }
            Log.d(TAG,"load favoriteApp time" + (System.currentTimeMillis()-time) + "ms");
            return true;
        }
        ...
        private void loadWorkspace() {
            ...
            if(DEBUG_LOADERS) Log.d(TAG, "is SafeMode:"+isSafeMode);
            mItems.clear();
            mAppWidgets.clear();
            mFolders.clear();
    ...
    public boolean addWidgetRumtime(AppWidgetProviderInfo appWidgetInfo , int appWidgetId, 
            LauncherAppWidgetInfo launcherInfo){        
        Callbacks callbacks = mCallbacks != null ? mCallbacks.get() : null;
        CellLayout layout = callbacks != null ? callbacks.getCellLayout(0) : null;
        if(callbacks != null && layout != null ){
            int[] spans = layout.rectToCell(appWidgetInfo.minWidth, appWidgetInfo.minHeight);
            int[] xy = new int[2];
            CellLayout.CellInfo cellInfo = layout.findAllVacantCells(null, null);
            if(!cellInfo.findCellForSpan(xy, spans[0], spans[1])){
                //Toast.makeText(mApp, getString(R.string.out_of_space), Toast.LENGTH_SHORT).show();
                return false;
            }
            launcherInfo.spanX = spans[0];
            launcherInfo.spanY = spans[1];

            LauncherModel.addItemToDatabase(mApp, launcherInfo,
                    LauncherSettings.Favorites.CONTAINER_DESKTOP,
                    callbacks.getCurrentWorkspaceScreen(), xy[0], xy[1], false);
            callbacks.bindAppWidget(launcherInfo);
            return true;
        }
        return false;
    }
    ...
    private ArrayList<ComponentName> mFilterList = new ArrayList<ComponentName>();
    /**
     * it can get a filtrate list from a origin app list
     * @param originList the origin list of apps you want to filtrate 
     * @param contain true is include then remove and false is include then add   
     * @return the filtrate list
     * @note:access in UI thread
     */
    public ArrayList<ApplicationInfo> appFiltrate(ArrayList<ApplicationInfo> originList,boolean contain){
        if(originList == null){
                return new ArrayList<ApplicationInfo>();
        }
        ArrayList<ApplicationInfo> filterList;
        synchronized(mFilterList){
            if(contain){             
                filterList = (ArrayList<ApplicationInfo>) originList.clone();
                for(ApplicationInfo app:filterList){
                    if(mFilterList.contains(app.componentName)){
                        filterList.remove(app);
                    }
                }
            }else{
                filterList = new ArrayList<ApplicationInfo>();
                for(ApplicationInfo app:originList){
                    if(mFilterList.contains(app.componentName)){
                        filterList.add(app);
                    }
                }

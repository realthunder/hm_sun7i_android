 ...
 * you may not use this file except in compliance with the License.
    chmod 777 /dev/ttyS5 * You may obtain a copy of the License 
 *
...
static void 
sys_pin_output(char *pin_name, int value)
{
    int fd = open(pin_name, O_RDWR);
    if(fd <= 0){
        LOGE("error in opening gpio node = %s.\n", pin_name);
        return;
    }
    
    LOGD("output %d in pin %s.\n", value, pin_name);
    if(value){
        write(fd, "1", strlen("1"));
    }else{
        write(fd, "0", strlen("0"));
...
gps_state_start( GpsState*  s )
{
    char  cmd = CMD_START;
    int   ret;
    char *gps_reset_pin = "/sys/class/gpio_sw/PA00/data";
    char *gps_vbat_pin = "/sys/class/gpio_sw/PA01/data";
    ...
    //sys_pin_output(gps_vbat_pin, 1);
   // sys_pin_output(gps_reset_pin, 1);
}
...
gps_state_stop( GpsState*  s )
{
    ...
    if (ret != 1)
        D("%s: could not send CMD_STOP command: ret=%d: %s",
          __FUNCTION__, ret, strerror(errno));
    //sys_pin_output(gps_reset_pin, 0);
    //sys_pin_output(gps_vbat_pin, 0);
}
...
//    state->fd = qemud_channel_open(QEMU_CHANNEL_NAME);
    state->fd = open(TTY_NAME, O_RDWR | O_NONBLOCK | O_NOCTTY);         // UART5

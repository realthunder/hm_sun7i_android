...
struct mixer_ctls
{
    ...
    struct mixer_ctl *audio_mic1_in;
    struct mixer_ctl *audio_mic2_in;
};
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
	...
	ret = str_parms_get_str(parms, AUDIO_PARAMETER_STREAM_INPUT_DEVICE, value, sizeof(value));

    if (ret >= 0)
    {
        ALOGV("in AUDIO_PARAMETER_STREAM_INPUT_DEVICE: %s", value);
       if(!strcmp(value,"4"))//ear mic 
       	{
		   ret = str_parms_get_str(parms, AUDIO_PARAMETER_STREAM_INPUT_STATE_DEVICE, value, sizeof(value));
		   ALOGV("in AUDIO_PARAMETER_STREAM_INPUT_STATE_DEVICE: %s,RET=%d", value,ret);
		   if(ret >= 0 && !strcmp(value,"1"))
		   	{
        	mixer_ctl_set_value(adev->mixer_ctls.audio_mic2_in, 0, 1);
		mixer_ctl_set_value(adev->mixer_ctls.audio_mic1_in, 0, 0);
		   ...
		   if( ret >= 0 && strcmp(value,"1"))
			{
				//ear mic plug out
        	mixer_ctl_set_value(adev->mixer_ctls.audio_mic2_in, 0, 0);
        	mixer_ctl_set_value(adev->mixer_ctls.audio_mic1_in, 0, 1);
			}
                     ...
                     hw_device_t** device)
{
    ...
    adev->mixer_ctls.audio_mic1_in = mixer_get_ctl_by_name(adev->mixer,
									   "Audio phone phonemic");
	if (!adev->mixer_ctls.audio_mic1_in) {
		ALOGE("Unable to find Audio phone phonemic  mixer control\n");
		goto error_out;
	}

    adev->mixer_ctls.audio_mic2_in = mixer_get_ctl_by_name(adev->mixer,
									   "Audio phone headsetmic");
	if (!adev->mixer_ctls.audio_mic2_in) {
		ALOGE("Unable to find Audio phone headsetmic  mixer control\n");
		goto error_out;
	}
	mixer_ctl_set_value(adev->mixer_ctls.audio_mic1_in, 0, 1);
